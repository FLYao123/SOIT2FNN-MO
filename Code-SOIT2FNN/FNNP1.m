...
%             res_last = loss;
    ...
    % æž„å»ºæ�?��?�ä»¶å�??
    filename_weights = fullfile(subfolder_weights, sprintf('IT2FNNP1_%d_Chaotic_0.3_weights_I%d_M%d_C%d_Tg%2f_Tr%2f.mat', iii,I, M,Cnum,threshold_growing,threshold_reducing));
%     filename_weights = fullfile(subfolder_weights, sprintf('IT2FNNP1_%d_unmets_weights_I%d_M%d_C%d_Tg%2f_Tr%2f.mat', iii,I, M,Cnum,threshold_growing,threshold_reducing));    
...
for iii = 1: MM
%     variable_name_ant = ['M', num2str(iii), '_ant_par'];
%     current_value_ant = eval(variable_name_ant);
% 
%     variable_name_con = ['M', num2str(iii), '_con_par'];
%     current_value_con = eval(variable_name_con);
% 
%     variable_name_Mqlqr = ['M', num2str(iii), '_Mqlqr'];
%     current_value_Mqlqr = eval(variable_name_Mqlqr);

    filename_weight_loading = fullfile(subfolder_weights, sprintf('IT2FNNP1_%d_prs_weights_I9_M1_C10_Tg0.001000_Tr0.001000.mat',iii));
%     filename_weight_loading = fullfile(subfolder_weights, sprintf('IT2FNNP1_%d_Chaotic_0.0_weights_I9_M1_C5_Tg0.002500_Tr0.002500.mat',iii));
    load(filename_weight_loading);

%     [My_Y_tmp] = IT2FNNP (X_test,current_value_ant,current_value_con,current_value_Mqlqr,I,Y(:,iii));
    [My_Y_tmp] = IT2FNNP (X_test,ant_par,con_par,Mqlqr,I,Y_test(:,iii),Mant, K_link,Mylyr);

    My_Y(:,iii)= My_Y_tmp;  % 1* sam_num;å®ŒæˆåŽæ?»å…±ä¸ºMM* sam_num

    mse= mean((My_Y(:,iii) - Y_test(:,iii))'.^2, 2); % Y_batchå�?? M_y ä¸? batch_size*Mï¼? sum((Y_batch' - M_y).^2, 2)ä¸ºM*1; 
    disp('loss(mse): ');
    disp(mse)
    ...
    if If_normalization == 1
        Y_back(:,iii) = Y_test(:,iii)*(max_vals-min_vals) + min_vals;  
        My_y_back(:,iii) = My_Y(:,iii)*(max_vals-min_vals) + min_vals;
    else
        Y_back(:,iii) = Y_test(:,iii);  
        My_y_back(:,iii) = My_Y(:,iii);
    end

    figure(iii+1)
    plot(Y_back(:,iii), 'b', 'LineWidth', 2);  % 'r-' è¡¨ç¤ºçº¢è‰²å®žçº�?
    hold on; 
...
% ya_pm = Y_back;
% 
% error_pm = ya_pm - yp_pm;
% r_error_pm = abs(ya_pm - yp_pm)./ya_pm;
% 
% 
% subfolder_weights = 'Post_processing';
% filename_results = fullfile(subfolder_weights, 'prs_results_PM.mat');
% % filename_results = fullfile(subfolder_weights, 'Chaotic_results_PM.mat');
% save(filename_results, 'error_pm','r_error_pm','yp_pm','ya_pm');
